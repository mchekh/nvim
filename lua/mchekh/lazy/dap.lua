                ...
                handlers = {
                    ...
                    node2 = function(config)
                        table.insert(config.configurations, 1, {
                            type = "node2",
                            request = "launch",
                            name = "Debug custom Node command",
                            runtimeExecutable = "node",
                            runtimeArgs = function()
                                local cmd = vim.fn.input("Command to debug (e.g. npm run test:unit): ")
                                local args = vim.split(cmd, " ")
                                -- Wrap with --inspect-brk for debugging
                                return vim.list_extend({ "--inspect-brk" }, args)
                            end,
                            cwd = vim.fn.getcwd(),
                        ...
                        table.insert(config.configurations, 1, {
                            type = "node2",
                            request = "launch",
                            name = "Debug custom npm/yarn command",
                            runtimeExecutable = "npm", -- or "yarn"
                            runtimeArgs = function()
                                local cmd = vim.fn.input("npm/yarn args (e.g. run test:unit): ")
